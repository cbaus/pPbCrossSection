...
void makePlots_pt_cuts_eff_pur(bool draw=1, string filename = "histos_deleteme.root");


void makePlots_pt_cuts_eff_pur(bool draw, string filename)
{
  ...
  for(int n=0; n<int(type.size()); n++)
    {
      ...
      for (int i = 0; i<imax; ++i)
        {
          ...
          histeff->GetYaxis()->SetRangeUser(0.85,1.001);
          histpur->GetYaxis()->SetRangeUser(0.85,1.001);

          histeff->GetXaxis()->SetTitle("p_{T} cut [GeV]");
          histpur->GetXaxis()->SetTitle("p_{T} cut [GeV]");
      ...
      canvases[n][1]->SaveAs((string("plots/hadron_ptcuts_eff_pur_combined_") + type[n] + string(".pdf")).c_str());
      canvases[n][1]->SaveAs((string("plots/hadron_ptcuts_eff_pur_combined_") + type[n] + string(".png")).c_str());
