      ...
      for(int iEvent=0; iEvent<n_total; iEvent++)
        {
          ...
          hf_single_energy_max = hf_p_energy_max;
          if(event->Tracks.size()>0)
            {
              hf_double_energy_max = 0;
              hf_single_energy_max = 0;
            }//do not select any event that has a track. this is for diffractive enhancement only
