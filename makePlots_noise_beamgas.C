...
void makePlots_noise_beamgas()
{
  ...
  Show("hf", noise, beamgas, "BPTX+XOR-");

  filename_ss.str(""); filename_ss << "data247324/data247324_h_castor_sumE_noise";
  cout << filename_ss.str() << endl;
  TH1D* noise=(TH1D*)file->Get(filename_ss.str().c_str());
  filename_ss.str(""); filename_ss << "data247324/data247324_h_castor_sumE_beam_plus";
  cout << filename_ss.str() << endl;
  TH1D* plus=(TH1D*)file->Get(filename_ss.str().c_str());
  filename_ss.str(""); filename_ss << "data247324/data247324_h_castor_sumE_beam_minus";
  cout << filename_ss.str() << endl;
...
void Show(string prefix, TH1D* noise, TH1D* beamgas1, string title1, TH1D* beamgas2, string title2)
{
  TCanvas* can1 = new TCanvas;
      ...
      //integral = 1;
      noise->Scale(1./noise->Integral());
      if(beamgas1) beamgas1->Scale(1./beamgas1->Integral());
      if(beamgas2) beamgas2->Scale(1./beamgas2->Integral());
      // int rebin = 50;
      // noise->Rebin(rebin);
      ...
      if(beamgas1) beamgas1->GetYaxis()->SetRangeUser(1e-5,1.3);
      if(beamgas2) beamgas2->GetYaxis()->SetRangeUser(1e-5,1.3);
      ...
      ostringstream ss_title_noise; ss_title_noise << "No BPTX (overflow: " << scientific << setprecision(0) << overflow_noise << ");#it{E} [GeV];event fraction";
      ostringstream ss_title_beamgas1; if(beamgas1) ss_title_beamgas1 << title1 << " (overflow: " << scientific << setprecision(0) << overflow_beamgas1 << ");#it{E} [GeV];event fraction";
      ostringstream ss_title_beamgas2; if(beamgas2) ss_title_beamgas2 << title2 << " (overflow: " << scientific << setprecision(0) << overflow_beamgas2 << ");#it{E} [GeV];event fraction";
  ...
  CMSText(3,prefix=="hf"?1:1,prefix=="hf"?0:0,prefix=="hf"?"HF hottest tower (single-arm)":"CASTOR (sum first 5 mod)","Random Trigger");
